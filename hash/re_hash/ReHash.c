...
int ReHash(HashTable *pHashTable)
{
    assert(NULL != pHashTable);

    HashNode oldTable = pHashTable->table;
    int oldLength = pHashTable->length;
    int oldSize = pHashTable->size;

    if (Init(pHashTable, 2 * oldLength) != 0)
    {
        return -1;
    }
    ...
    {
        if (Node_Full == oldTable[i].status)
        {
            Insert(pHashTable, oldTable[i].element);
        }
    }
    pHashTable->size = oldSize;
    free(oldTable);
    oldTable = NULL;
...
int Insert(HashTable *pHashTable, const char *key)
{
    ...
    // 若找不到插入地方则循环
    while (Node_Full == insertNode->status)
    {
        /**
         * 若已有重复元素则插入失败
         * 若无重复元素则散列
         */
        if (strcmp(insertNode->element, key) == 0)
        {
            return -1;
        }
        else
        {
            if (ReHash(pHashTable) != 0)
            {
                return -1;
            }
            insertNode = Find(pHashTable, key);
        }
    }
